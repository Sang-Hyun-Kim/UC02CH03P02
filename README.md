...
- 중력을 정말 간단히 구현하였지만, 내가 원했던 중력 및 운동에너지 벡터는 착지했든, 체공중이든 계속해서 적용되어야한다고 생각했다. 따라서 이를 현재 벡터의 총량을 적용시키는 방법으로 구현하고 싶었으나 현재 기초적인 언리얼 지식으로는 구현하지 못해 단순히 Location이 변화하는 것으로만 구현했다. 이 방식이 아쉬웠다.
- 오일러 각과 쿼터니안의 차이를 공부해보았으나 수학적 지식이 좀 더 필요한 것 같다..
